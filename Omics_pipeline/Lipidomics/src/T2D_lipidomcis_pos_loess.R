...
library(metaX)
para <- new("metaXpara")
pfile <- "T2D_lipidomics_pos_LipidBlast_and_HMDB_measurement.csv"
sfile <- "DM_lipid_pos_sample.list"
#sfile <- "zaochan_globle_pos(batch1-6-QC29)_remove3.list"
idres <- "T2D_lipidomics_pos_LipidBlast_and_HMDB_identification.csv"

para@outdir <- "20151118T2D_pos_loess_nor"
para@prefix <- "pos"

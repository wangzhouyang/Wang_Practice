...
my $usage = "usage: perl $0 [options]
			...
			-cs     cv Steps
			-pf     prefix
			-R		Rscript path. Default \"Rscript\"
";

die $usage if @ARGV < 24;

my ($setTimes, $pheF, $todo, $proF, $do_wilcox, $todo_id, $todo_marker, $control, $case1, $case2, $fold, $cvTimes, $cvSteps, $prefix, $Rscript);
GetOptions(
	"s:s" => \$setTimes,
	"phe:s" => \$pheF,
	"todo:s"=> \$todo,
...
#	print SH "export /ifs1/ST_META/USER/xiehailiang/usr/R/ && ";
	...
	while($#vs >-1){
		...
		if ($do_wilcox = "T"){
			...
			print WC "$Rscript $Bin/wilcox_rf.r $repeat_pfx.$v.train_x.xls $repeat_pfx.$v.train_y.xls $wilcox_marker $v &\n";
			$todo_marker = $wilcox_marker;
		}
		print SH "$Rscript $Bin/randomForest_CRC.R $repeat_pfx.$v.train_x.xls $repeat_pfx.$v.train_y.xls $repeat_pfx.$v.test_x.xls $repeat_pfx.$v.test_y.xls $todo_marker 10 0.9 5 0 $repeat_pfx.$v 1>$repeat_pfx.$v.stat1 2>$repeat_pfx.$v.stat2 \n";
		#print SH "$Rscript $Bin/randomForest_FC.R $wd/$prefix.$v.train_x.xls $wd/$prefix.$v.train_y.xls $wd/$prefix.$v.test_x.xls $wd/$prefix.$v.test_y.xls 10 0.9 5 0 $wd/$prefix 1>$wd/$prefix.stat1 2>$wd/$prefix.stat2 \n";
	}
}
print WC "wait\n";
close WC;
